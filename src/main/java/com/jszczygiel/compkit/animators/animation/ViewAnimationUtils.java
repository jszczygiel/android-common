package com.jszczygiel.compkit.animators.animation;
...
public final class ViewAnimationUtils {
  ...
  /**
   * Returns an Animator which can animate a clipping circle. <p> Any shadow cast by the View will
   * respect the circular clip from this animator. <p> Only a single non-rectangular clip can be
   * applied on a View at any time. Views clipped by a circular reveal animation take priority
   * over {@link View#setClipToOutline(boolean) View Outline clipping}. <p> Note that the
   * animation returned here is a one-shot animation. It cannot be re-used, and once started it
  ...
  public static Animator createCircularReveal(View view, int centerX, int centerY,
    ...
    if (!rm.hasCustomerRevealAnimator() && Build.VERSION.SDK_INT >= Build.VERSION_CODES
        .LOLLIPOP) {
      return android.view.ViewAnimationUtils.createCircularReveal(view, centerX, centerY,
          startRadius, endRadius);
    }

    ViewRevealManager.RevealValues viewData = new ViewRevealManager.RevealValues(view, centerX,
        centerY, startRadius, endRadius);
    ObjectAnimator animator = rm.createAnimator(viewData);

    if (layerType != view.getLayerType()) {
      animator.addListener(
          new ViewRevealManager.ChangeViewLayerTypeAdapter(viewData, layerType));
    }
    return animator;
  }
}
