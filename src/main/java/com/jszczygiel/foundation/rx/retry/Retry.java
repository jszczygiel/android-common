...
public class Retry {
   ...
   * @param maxAttempts the maximum number of times to attempt a retry. It will be capped at <code>
   *     {@link Integer#MAX_VALUE} - 1</code>.
   ...
   * by the {@link RetryWithDelayHandler}, which can also filter on errors and stop the retry cycle
   * for certain type of errors.
   *
   * @param source the {@link Observable} to wrap.
   ...
   * @return the wrapped retrying Observable.
   */
  public static <T> Observable<T> wrapForRetry(
      Observable<T> source, final RetryWithDelayHandler handler) {
   ...
   */
  public static <T> Observable<T> wrapForRetry(
      Observable<T> source, int maxAttempts, Delay retryDelay) {
   ...
   * @return an Observable that combines the index/attempt number of each error with its error in a
   *     {@link Tuple}.
   */
  protected static Observable<Tuple<Integer, Throwable>> errorsWithAttempts(
      Observable<? extends Throwable> errors, final int expectedAttempts) {
